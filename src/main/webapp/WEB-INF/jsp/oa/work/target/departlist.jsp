...
$(function() {
	...
	function initChart() {
		...
		$.post("oa/work/target/getChartData", {select:'3'}, function(o){
			myChart.hideLoading();
	...
	function goPage(currentPage) {
		...
		$.post("oa/work/target/getDepartData",
						{
							page : currentPage,
							rows : 20,
							field : field,
	...
	function doBtnNewAction() {
		parent.layer.open({
			...
			shade : 0.8,
			area : [ '50%', '70%' ],
			content : 'oa/work/target/addorupdatedepart',
			btn : [ '确定', '取消' ],
	...
	function doBtnEditAction() {
		...
		parent.layer.open({
			...
			shade : 0.8,
			area : [ '50%', '70%' ],
			content : 'oa/work/target/addorupdatedepart?id=' + id,
			btn : [ '确定', '取消' ],
			yes : function(index, layero) {
				//确定按钮回调,表单提交
				parent.frames['layui-layer-iframe' + index].submitL();
